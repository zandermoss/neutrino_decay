...
E = np.asarray([4.0e12])
amps=[]
ang_arr=[1.0,0.5,0.3,0.2,0.1,0.05,0.01]
for ang in ang_arr:
	print ang*param.PI
	dists,amp= sp.OscHistory(1,1,E,ang*param.PI,dcy_channels,tau,param,pg,nu_mass,phi_mass,regen,mtr_switch)
	amps.append(amp)


np.savez(calcvar+"_"+regenarg+"_prophist_"+mattervar,_E=E,_potential=potential,_valvec=valvec,_dists_=dists,_num_vec=amps,_ntype=ntype,ang_arr=ang_arr)
