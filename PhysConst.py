...
class PhysicsConstants(object):
	...
	def __init__(self):
		...
		self.dcps = np.zeros([self.numneumax-2+1])
		self.dcpc = np.zeros([self.numneumax-2+1])

	
		for i in range(0,2):	
			self.dcps[i]=np.sin(self.dcp[i])
			self.dcpc[i]=np.cos(self.dcp[i])
